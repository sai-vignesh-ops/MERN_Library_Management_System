...
//how much time have been passed since overdue
...
};//to calculate fine according to the overdue days

//Issue a maual book to a student
...
export async function getStudentIssues(req, res) {
    try {
        ...
        console.error("Error fetching the students issues:", error)
        res.status(500).json({
            message: "Error fetching the students issues:", error: error.message
        });
    }
}
//4.Return issued manual books
export async function returnBook(req, res) {
    try {
        ...
        console.error("Error returning maunal Books:", error)
        res.status(500).json({
...
export async function clearFine(req, res) {
    try {
        ...
        console.error("Error clearing maunal Fine:", error)
        res.status(500).json({
            message: "Error clearing manual fine:", error: error.message
        });
    }
}
//7.Get Fine active settings
