...
import { AlertTriangle, ShieldEllipsis, UserRound, ArrowRight, Activity } from 'lucide-react';
                ...
                <div className={s.heroInner}>
                    <div>
                        <span className={s.badge}>
                            Collge administration workspace</span>
                        <h1 className={s.heading}>
                            Manage issued books,student records,returns,overdue status,and fines.
                        </h1>
                        <p className={s.heroParagraph}>
                            The admin area now focuses on visual trend graphs while keeping the existing admin worflows unchanged.
                        </p>
                    </div>
                </div>
            </section>
                ...
                <div className={s.overdueHeader}>
                    <div>
                        ...
                        <p className={s.overdueSubtitle}>
                            Top 4 overdue students ranked by total imposed fine,with highest fine shown first
                        </p>
                    </div>
                ...
                <div className={s.overdueGrid}>
                    {attentionRecords.length ? (
                        attentionRecords.map((record, index) => (
                            <div key={`${record.mail}-${record.id}`} className={s.overdueCard}>
                                {index === 0 ? (
                                ...
                                <div className={s.overdueCardInner}>
                                    <div>
                                        ...
                                        <p className={s.studentDetails}>
                                            {record.studentId ?? "No assigned"} | {record.email}
                                        </p>
                                        <p className={s.studentFine}>
                                            Rs.{record.totalFine}
                                        </p>
                                    </div>
                                    <div className={s.highestBookFineContainer}>
                                    ...
                                    <div className={s.deatailItem}>Book Code:{record.bookCode}</div>
                                    <div className={s.detailItem}>
