...
export const LibraryProvider = ({ children }) => {
    ...
    const fetchManualIssues = async () => {
        ...
        try {
            ...
            const issuesUrl = isAdmin ? `${API_BOOKS_URL}/issues` : `${API_BOOKS_URL}/issue/student`;
            const response = await fetch(issuesUrl, {
                headers: getHeaders(),
            });
            console.log("DEBUG fetchManualIssues:", response.status, issuesUrl);
    ...
    console.log("DEBUG issues:", allRecords.map((r) => ({ userEmail: r.userEmail, title: r.title })));
    console.log("DEBUG accounts:", studentAccounts.map((a) => ({ email: a.email, name: a.name })));
        ...
        ? studentSummaries.find((student) => student.email === currentUser.email) ?? (() => {
            const records = allRecords
                .filter(
                    (record) =>
                        (record.userEmail ?? "").toLowerCase().trim() ===
                        (currentUser.email ?? "").toLowerCase().trim(),   // ← bug is here
                )
                .sort((a, b) => new Date(b.issuedOn).getTime() - new Date(a.issuedOn).getTime());
